...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    /***************** weight  LUT **********************/
    int maxRange = nChannel * (255 + 10) * (255 + 10);  // we quantize the intensity values of the guidance image into 65025 bins, 
                                                                                  // but their values shoud meet |I_max - I_min|<=1
    WeightLUT = mxGetPr(mxCreateDoubleMatrix(1, maxRange, mxREAL));
    for(int i = 0; i < maxRange; i ++)  WeightLUT[i] = 1 / pow((double)i / (65025.0 * (double)nChannel) + delta, alpha);
        ...
        for(i = 0; i < rowNum; i ++)
        {
            ...
            for(t = - rSmooth; t <= rSmooth; t += stride)
            {
                ...
                for(s = - rSmooth; s <= rSmooth; s += stride)
                {
                    ...
                    WeightPr[counter] = WeightLUT[(int)(diffSum * 65025.0)];
